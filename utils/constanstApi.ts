//https://tcn-server-contact-production.up.railway.app/api
// http://localhost:5000/api
// "https://work.contactamericas.com/api"
export const API_URL = "https://work.contactamericas.com/api";
// export const LINK_BACKEND ="https://work.contactamericas/api/";
export const LINK_BACKEND = "https://work.contactamericas.com/api/";

export const ENDPOINT_EMPLOYEE = "";
export const ENDPOINT_AUTH = "";
export const ENDPOINT_CLIENTS = "";
export const ENDPOINT_JOBS = "";
export const ENDPOINT_SERVICES = "";
export const ENDPOINT_LANGUAGES = "";
export const ENDPOINT_PROFESIONAL = "";
